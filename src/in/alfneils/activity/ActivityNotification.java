...
public class ActivityNotification extends SherlockPreferenceActivity implements OnPreferenceChangeListener {
	...
	@Override
	  public boolean onPreferenceChange(Preference paramAnonymousPreference, Object paramAnonymousObject) {
		 ActivityNotification.this.color = ColorPickerPreference.convertToARGB(Integer.valueOf(String.valueOf(paramAnonymousObject)).intValue());
		  paramAnonymousPreference.setSummary(ActivityNotification.this.color);
		  SharedPreferences.Editor localEditor = ActivityNotification.this.getSharedPreferences("Alfahmi", 0).edit();
		  localEditor.putString("warnaTracking", ActivityNotification.this.color);
        localEditor.commit();
        Intent localIntent = new Intent();
        localIntent.setAction("in.alfneils.WARNA_TRACKING");
		  localIntent.putExtra("kolor", ActivityNotification.this.color.toString());
		  ActivityNotification.this.sendBroadcast(localIntent);
        return false;
      }
    });
	String str = getSharedPreferences("Alfahmi", 0).getString("warnaTracking", "#ff8e979f");
